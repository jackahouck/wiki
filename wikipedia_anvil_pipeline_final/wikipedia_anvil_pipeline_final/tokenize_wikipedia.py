#!/usr/bin/env python3
"""Tokenize canonical cleaned Wikipedia with the exact bracket-model tokenizer.

Input:
    $PROJECT/training/wikipedia/cleaned

Tokenizer source (default):
    tokenizer_bracket/ next to this script

Output:
    $PROJECT/training/wikipedia/tokenized_gpt2_1024

Each article receives exactly one EOS token. Article token streams are
concatenated within train and validation separately, divided into exact
1,024-token blocks, and only the final incomplete block of each split is
dropped. The script checkpoints at Parquet-shard boundaries and can resume.
"""
...
DEFAULT_TOKENIZER_PATH = SCRIPT_DIR / "tokenizer_bracket"
...
def tokenizer_semantic_fingerprint(tokenizer: Any) -> str:
    """Hash the token-to-id map and special-token behavior, not just vocab size."""
    payload = {
...
def load_comparison_tokenizer(path: Path) -> tuple[Any, str]:
    if not path.is_dir():
        raise RuntimeError(
            f"Comparison tokenizer directory does not exist: {path}. "
            "Place tokenizer_bracket next to this script or set "
            "COMPARISON_TOKENIZER_PATH."
        )
    if not (path / "tokenizer.json").exists():
        raise RuntimeError(f"Missing tokenizer.json in {path}")

    tokenizer = AutoTokenizer.from_pretrained(str(path), local_files_only=True)
    if tokenizer.eos_token_id is None:
        raise RuntimeError("Comparison tokenizer has no EOS token ID")
    tokenizer.pad_token = tokenizer.eos_token
    fingerprint = tokenizer_semantic_fingerprint(tokenizer)

    print(f"Comparison tokenizer source: {path}", flush=True)
    print(f"Tokenizer base vocabulary size: {tokenizer.vocab_size:,}", flush=True)
    print(f"Tokenizer total vocabulary size: {len(tokenizer):,}", flush=True)
    print(f"Tokenizer added vocabulary entries: {len(tokenizer.get_added_vocab()):,}", flush=True)
    print(f"Tokenizer EOS/PAD ID: {tokenizer.eos_token_id}", flush=True)
    print(f"Tokenizer semantic SHA-256: {fingerprint}", flush=True)
    print(f"Added vocabulary sample: {list(tokenizer.get_added_vocab().items())[:20]}", flush=True)
...
def tokenize_split(
    ...
    expected_source_fingerprint = source_fingerprint(dataset)
    ...
    if state_path.exists():
        ...
        if state.get("source_fingerprint") != expected_source_fingerprint:
            raise RuntimeError(
                f"{split_name}: source dataset fingerprint changed. Use --force to rebuild."
            )
        if state.get("tokenizer_fingerprint") != tokenizer_fingerprint:
            raise RuntimeError(
                f"{split_name}: tokenizer mapping changed. Use --force to rebuild."
            )
        if state.get("vocab_size") != expected_vocab_size:
            raise RuntimeError(
                f"{split_name}: tokenizer vocabulary size changed. Use --force to rebuild."
            )
...
def validate_fixed_length(dataset_dict: DatasetDict) -> None:
    ...
    for split_name, split in dataset_dict.items():
        ...
        for index in [0, len(split) // 2, len(split) - 1]:
            ...
            print(
                f"  sample row {index:,}: length={len(ids)}, first_ids={ids[:12]}",
                flush=True,
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--tokenizer-path",
        type=Path,
        default=Path(os.environ.get("COMPARISON_TOKENIZER_PATH", DEFAULT_TOKENIZER_PATH)),
        help="Exact tokenizer_bracket directory used by the bracket model",
    )
    parser.add_argument(
    ...
    tokenization_info_path = output_dir / "TOKENIZATION_INFO.json"
    ...
    source_tokenizer_path = args.tokenizer_path.expanduser().resolve()
    tokenizer, tokenizer_fingerprint = load_comparison_tokenizer(source_tokenizer_path)

    if output_dir.exists() and (output_dir / "_SUCCESS").exists() and not args.force:
        if not tokenization_info_path.exists():
            raise RuntimeError(
                f"Completed output lacks TOKENIZATION_INFO.json: {output_dir}. "
                "Use --force to rebuild."
            )
        metadata = json.loads(tokenization_info_path.read_text(encoding="utf-8"))
        if metadata.get("tokenizer_fingerprint") != tokenizer_fingerprint:
            raise RuntimeError(
                "Existing tokenized Wikipedia was made with a different tokenizer. "
                "Run tokenize_wikipedia.py --force."
            )
    ...
    if set(cleaned.keys()) != {"train", "validation"}:
        raise RuntimeError(f"Unexpected cleaned splits: {list(cleaned.keys())}")
    ...
    # Save an immutable experiment copy, then verify it behaves identically.
    shutil.rmtree(tokenizer_dir, ignore_errors=True)
    tokenizer.save_pretrained(str(tokenizer_dir))
    saved_tokenizer = AutoTokenizer.from_pretrained(str(tokenizer_dir), local_files_only=True)
    saved_tokenizer.pad_token = saved_tokenizer.eos_token
    saved_fingerprint = tokenizer_semantic_fingerprint(saved_tokenizer)
    if saved_fingerprint != tokenizer_fingerprint:
        raise RuntimeError("Saved comparison-tokenizer copy changed token mappings")
    print(f"Saved exact tokenizer copy to: {tokenizer_dir}", flush=True)
    ...
    metadata = {
        ...
        "saved_tokenizer_path": str(tokenizer_dir),
        "tokenizer_fingerprint": tokenizer_fingerprint,
        ...
        "note": (
            "Control text was tokenized with the exact bracket-model tokenizer. "
            "Only input_ids are stored. The training collator creates labels without "
            "masking real EOS article-boundary tokens."
        ),
    }
